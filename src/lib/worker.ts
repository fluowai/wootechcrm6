...
// Only starts when Redis is available
// ============================================================

if (!redisConnection) {
  console.warn('[ScraperWorker] Redis not configured — worker disabled');
} else {
  const scraperWorker = new Worker(
      ...
      async (job: Job) => {
          ...
          if (job.name === 'google-maps-scrape') {
              ...
              console.log(`[ScraperWorker] Gosom retornou ${results.length} leads.`);
